...
def create_model(num_zones:int, num_SRs:int, current_assignment: dict, distances:pd.DataFrame, index_values:pd.Series, wl_interval:tuple[float, float]=(0.8, 1.2)):
    center_brick = {sr: data["Center brick"] for sr, data in current_assignment.items()}
...
if __name__=='__main__':
    model = create_model(num_zones=num_zones, num_SRs=num_SRs, current_assignment=current_assignment, distances=distances, index_values=index_values)
    model.optimize()
