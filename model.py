...
def workload_constraints(
    ...
    for sr in range(num_SRs):
        ...
        model.addConstr(
            quicksum(
                SR_matrix[zone, sr] * index_values[zone] for zone in range(num_zones)
            )
            <= wl_max,
...
def create_model(
    ...
    workload_constraints(
        ...
        wl_min=wl_interval[0],
        wl_max=wl_interval[1],
    )
...
if __name__ == "__main__":
    ...
    plot_cities_attribution(
        get_solution_dict(
            ...
            num_zones,
            num_SRs,
            [current_assignment[i]["Center brick"] for i in current_assignment],
        )
    )
