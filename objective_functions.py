...
def disruption(SR_matrix:MVar, boolean_matrix:MVar, current_assignment: dict, distances:pd.DataFrame, index_values:pd.Series):
...
def distance(SR_matrix:MVar, boolean_matrix:MVar, current_assignment: dict, distances:pd.DataFrame, index_values:pd.Series):
    num_zones, num_SRs = SR_matrix.shape
    objective = 0
